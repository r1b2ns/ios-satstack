...
///
/// Node lifecycle (start, stop, status) is centralised in `KyotoNodeManager.shared`.
/// This service focuses on wallet loading, descriptor construction, and delegating
/// sync / broadcast calls to the singleton manager.
struct KyotoWalletService: WalletServiceProtocol {
    ...
    func fetchWalletBalance(for wallet: Wallet, onProgress: @escaping @Sendable (Double?) -> Void) async throws -> UInt64 {
        ...
        let needsRecovery = Self.needsFullScan(for: wallet.id)
        let scanType: ScanType = needsRecovery
            ? .recovery(usedScriptIndex: 1000, checkpoint: .segwitActivation)
            : .sync

        try await KyotoNodeManager.shared.syncWallet(
            bdkWallet, persister: persister, walletId: wallet.id,
            scanType: scanType, onProgress: onProgress
        )

        if needsRecovery { Self.markFullScanCompleted(for: wallet.id) }
    ...
    func fetchWalletTransactions(for wallet: Wallet) async throws -> [WalletTransaction] {
        ...
        try await KyotoNodeManager.shared.syncWallet(
            bdkWallet, persister: persister, walletId: wallet.id,
            scanType: scanType, onProgress: { _ in }
        )
    ...
    func fullScanWallet(_ wallet: Wallet, onProgress: @escaping @Sendable (Double?) -> Void) async throws -> (balance: UInt64, transactions: [WalletTransaction]) {
        ...
        let (bdkWallet, persister) = try loadBDKWallet(for: wallet)
        let scanType: ScanType = .recovery(usedScriptIndex: 1000, checkpoint: .segwitActivation)
    ...
    func broadcastTransaction(
        ...
        // Broadcast via the singleton CBF node for true P2P broadcasting.
        _ = try await KyotoNodeManager.shared.broadcast(
            transaction: tx, bdkWallet: bdkWallet, walletId: wallet.id
        )
